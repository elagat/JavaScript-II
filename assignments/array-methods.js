...
ticketPriceTotal = runners.reduce(function(counter, runner) {
  return counter + runner.donation;
}, 0)
...
// Problem 1: Collect the emails of all the people with shirt size L to let them know they need to change thier shirt size decision.
let emailAddress = [];
largeShirts.forEach(item => emailAddress.push(item.email));
console.log(emailAddress);

// Problem 2: Make an array with both first names and last names all capitalized.
let nameCaps = [];
fullName.map(item => nameCaps.push(item.toUpperCase()));
console.log(nameCaps);

// Problem 3: Get the company names for the comapanies that donated over $500.
let largeDonations = []
runners.filter(item => {
  if (item.donation >= 200) {
    largeDonations.push(item.company_name)
  }
});
